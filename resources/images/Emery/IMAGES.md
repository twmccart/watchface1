...
Emery is a color display (200×228px), so many images that were 1-bit grayscale on diorite are now **indexed color PNGs**. In indexed files, the Pebble SDK encodes transparency inverted: foreground/content pixels are transparent (alpha=0) and the background is opaque — intentional for Pebble's compositor. Corrected `-fixed.png` versions are provided for each indexed image, with the transparency inverted so content pixels are opaque and the background is transparent, suitable for standard compositing workflows.
...
| `IMAGE_HEART.png` / `IMAGE_HEART-fixed.png` | 15×20 | Heart rate indicator icon. Indexed PNG — use the `-fixed` version for standard compositing. |

---

## Weather icons

All weather icons are **15×20px**. Many are **indexed color PNGs** (emery's color display variants, sourced from `-200w.png` files); those have a `-fixed.png` with corrected transparency. The remainder are 1-bit grayscale.

| File | Type | Condition |
|------|------|-----------|
| `WEATHER_CLEAR_DAY.png` / `-fixed.png` | Indexed | Clear sky, daytime |
| `WEATHER_CLEAR_NIGHT.png` / `-fixed.png` | Indexed | Clear sky, nighttime |
| `WEATHER_PARTLY_CLOUDY_DAY.png` / `-fixed.png` | Indexed | Partly cloudy, daytime |
| `WEATHER_PARTLY_CLOUDY_NIGHT.png` / `-fixed.png` | Indexed | Partly cloudy, nighttime |
| `WEATHER_CLOUDY.png` | Grayscale | Overcast / cloudy |
| `WEATHER_RAIN.png` / `-fixed.png` | Indexed | Rain |
| `WEATHER_DRIZZLE.png` / `-fixed.png` | Indexed | Drizzle / light rain |
| `WEATHER_SLEET.png` | Grayscale | Sleet |
| `WEATHER_SNOW.png` | Grayscale | Snow |
| `WEATHER_SNOW_SLEET.png` | Grayscale | Snow and sleet mix |
| `WEATHER_RAIN_SNOW.png` / `-fixed.png` | Indexed | Rain and snow mix |
| `WEATHER_RAIN_SLEET.png` / `-fixed.png` | Indexed | Rain and sleet mix |
| `WEATHER_THUNDER.png` / `-fixed.png` | Indexed | Thunderstorm |
| `WEATHER_WIND.png` | Grayscale | Wind / blustery |
| `WEATHER_FOG.png` | Grayscale | Fog / mist |
| `WEATHER_HOT.png` / `-fixed.png` | Indexed | Extreme heat |
| `WEATHER_COLD.png` / `-fixed.png` | Indexed | Extreme cold |
| `ICON_CLOUD_ERROR.png` / `-fixed.png` | Indexed | Weather data unavailable / error |
